            ...
            children=[
                ...
                html.Div(
                    [
                        ...
                        dcc.Dropdown(
                            id="f-mes", multi=True, placeholder="Все коды услуг..."
                        ),
                    ],
                    className="mb-4",
                ),
...
def update_smart_filters(years, quarters, months, depts, profiles, mes_list):
    ...
    if "Наименование отделения" in df.columns:
        opts_dept = sorted(
            df.loc[get_mask_for("Dept"), "Наименование отделения"].dropna().unique()
        )
...
def update_standard_dashboard(
    years, quarters, months, depts, profiles, mes_list, metric, group_by_col, theme
):
    df = get_optimized_data()
    ...
    try:
        if not filtered_df.empty and "dt" in filtered_df.columns:
            ...
            if metric == "sum":
                trend_df = (
                    filtered_df.groupby("YearMonth", observed=True)["Сумма"]
                    .sum()
                    .reset_index()
                )
                metric_name = "Сумма"
                ...
                trend_df = (
                    filtered_df.groupby("YearMonth", observed=True)[
                        "ИД пациента в версии счета"
                    ]
                    .nunique()
            ...
            if trend_df.empty:
                insight_html = html.Div(
                    [
                        html.I(
                            ...
                            style={"color": "#FF7D00", "marginRight": "12px"},
                        ),
                        html.Span(
                            "Данные отсутствуют.", style={"color": "var(--text-muted)"}
                        ),
                    ]
                )
            else:
                ...
                for _, row in trend_df.iterrows():
                    ...
                    badge = html.Div(
                        [
                            ...
                            html.Span(
                                f"{fmt(val)}", style={"color": "var(--text-main)"}
                            ),
                            mom_element,
                        ],
                        style={
                ...
                if len(trend_df) >= 2:
                    ...
                    diff_block = html.Div(
                        [
                            ...
                            html.Span(
                                [
                                    html.I(
                                        className=icon, style={"marginRight": "6px"}
                                    ),
                    ...
                    diff_block = html.Div(
                        [
                            html.Span(
                                "Выберите больше периодов для сравнения динамики.",
                                style={
                                    "color": "var(--text-muted)",
                                    "fontStyle": "italic",
                                },
    ...
    if (
        ...
        and "dt" in filtered_df.columns
        and group_by_col in filtered_df.columns
    ):
        ...
        if metric == "sum":
            trend = (
                filtered_df.groupby(["dt", group_by_col], observed=True)["Сумма"]
                .sum()
                .reset_index()
            )
            y_col = "Сумма"
            ...
            trend = (
                filtered_df.groupby(["dt", group_by_col], observed=True)[
                    "ИД пациента в версии счета"
                ]
                .nunique()
                .reset_index(name="val")
    ...
    if not filtered_df.empty:
        ...
        if agg_cols:
            if "Сумма" in filtered_df.columns:
                grid_df = (
                    filtered_df.groupby(agg_cols, observed=True)
            ...
            for col in grid_df.columns:
                ...
                column_defs.append(
                    {"field": col, "filter": col_filter, "sortable": True}
                )

            grid_theme = (
                "ag-theme-alpine" if theme == "light" else "ag-theme-alpine-dark"
            )
    ...
    return (
        fig,
        "Аналитика по времени (Нажмите на любую точку для детализации 🔍)",
        total_sum,
        total_patients,
        total_mes,
        active_depts,
...
def drilldown_modal(clickData, close_clicks, is_open, group_by_col, metric):
    ...
    if clickData and trigger_id == "main-line-chart":
        ...
        fig_modal.update_layout(
            ...
            paper_bgcolor="rgba(0,0,0,0)",
            xaxis=dict(
                showgrid=True, gridcolor="var(--grid-color)", range=[0, x_max_range]
            ),
...
def execute_custom_sql(n_clicks, query, theme):
    ...
    sql_insight = html.Div(
        [
            ...
            html.Span(
                "Запрос выполнен. Ознакомьтесь с запросом.",
