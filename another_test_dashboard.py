...
THEME_COLORS = {
    "light": {"text": "#1B2559", "grid": "#E2E8F0", "card": "#FFFFFF"},
...
def render_schema_badges() -> list[html.Span]:
    """Возвращает список бейджей колонок схемы для переиспользования."""
    return [
        html.Span(
            col,
...
try:
    ...
    logger.warning("Redis недоступен (%s). Используется FileSystemCache.", exc)
    safe_cache_dir = os.path.join(tempfile.gettempdir(), "clinical_cache")
    cache = Cache(
        app.server,
        config={
...
def shutdown_server():
    ...
    print("Вкладка браузера закрыта. Начинаю процедуру завершения...")

    time.sleep(0.5)

    print(f"Принудительно убиваю процесс {os.getpid()}...")
    if os.name == 'nt':
...
def monitor_heartbeat():
    ...
    print("Мониторинг запущен. Жду подключения браузера (15 сек)...")
    time.sleep(15) 
    ...
    while True:
        ...
        if diff > 15:
            last_heartbeat_time = time.time() + 9999 
...
<html>
    <head>
        ...
        <style>
            ...
            .btn-apply-dirty {
                animation: apply-pulse 1.5s infinite !important;
            ...
            @media print { .no-print { display: none !important; } body { background-color: white !important; } .card, .dbc-card { box-shadow: none !important; border: 1px solid #ddd !important; background-color: var(--card-bg) !important; } }

            /* 🚀 СКЕЛЕТНАЯ ЗАГРУЗКА */
            [data-dash-is-loading="true"] { position: relative; pointer-events: none; }
            [data-dash-is-loading="true"] > * { opacity: 0 !important; transition: opacity 0.1s; }
            [data-dash-is-loading="true"]::before {
                content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0;
                background: linear-gradient(90deg, var(--card-bg) 25%, var(--grid-color) 50%, var(--card-bg) 75%);
                background-size: 200% 100%; animation: skeleton-loading 1.5s infinite linear; z-index: 100; border-radius: inherit;
            }
        ...
        <footer>
            ...
            console.log("%c🩺 Clinical Dashboard\n%cРазработано с ❤️ и бессонными ночами. Если вы это читаете — логи пишутся нормально, а данные в безопасности!", "color: #4318FF; font-size: 20px; font-weight: bold;", "color: #01B574; font-size: 14px;");
                ...
                document.addEventListener('keydown', function(e) {
                    if (e.target && e.target.classList.contains('sql-editor')) {
                        ...
                        if (closeChars.includes(e.key) && start === end && val[start] === e.key) {
                            e.preventDefault(); e.target.selectionStart = e.target.selectionEnd = start + 1; return;
                        }
                        if (e.key === 'Tab') {
                            ...
                            e.target.dispatchEvent(new Event('input', { bubbles: true })); return;
                        }
                        if (pairs[e.key]) {
                            e.preventDefault(); const openChar = e.key; const closeChar = pairs[openChar];
                            if (start !== end) {
                                const selected = val.substring(start, end);
                                nativeSetter.call(e.target, val.substring(0, start) + openChar + selected + closeChar + val.substring(end));
                                e.target.selectionStart = start + 1; e.target.selectionEnd = end + 1;
                            } else {
                ...
                function animateCounter(element, endStr) {
                    ...
                    let startVal = 0; let duration = 1000; let startTime = null;
                    function step(currentTime) {
                        ...
                        else { element.textContent = endStr; setTimeout(() => { element._isAnimating = false; }, 50); }
                    }
                ...
                const observer = new MutationObserver((mutations) => {
                    ...
                    mutations.forEach(mutation => {
                        if (mutation.type === 'characterData' || mutation.type === 'childList') {
                            let target = mutation.target; if (target.nodeType === 3) target = target.parentElement;
                            if (target && target.classList && target.classList.contains('kpi-value')) {
                                let newVal = target.textContent.trim();
                                if (target._dashValue !== newVal && !target._isAnimating && newVal !== "0") {
                                    target._dashValue = newVal; animateCounter(target, newVal);
                                }
                            }
                        }
                    });
                ...
                setInterval(function() {
                    fetch('/heartbeat', {method: 'POST'}).catch(e => console.log('Heartbeat stopped'));
                }, 5000);
            </script>
        </footer>
    </body>
...
def _normalize_col_name(name: str) -> str:
    """Нормализует имя колонки (убирает переносы/повторные пробелы)."""
    return " ".join(str(name).replace("\n", " ").replace("\r", " ").split())

def get_optimized_data() -> pd.DataFrame:
    """Загружает и кэширует данные из SQLite с минимальной выборкой колонок."""
    if not os.path.exists(DB_NAME):
        return pd.DataFrame()
    ...
    with sqlite3.connect(DB_NAME) as conn:
        cur = conn.execute("PRAGMA table_info(medical_data)")
        raw_cols = [r[1] for r in cur.fetchall()]
        norm_map = {_normalize_col_name(c): c for c in raw_cols}

        select_real = [
            norm_map[c] for c in REQUIRED_COLUMNS if c in norm_map
        ]
        if not select_real:
            df = pd.read_sql("SELECT * FROM medical_data", conn)
        else:
            cols_sql = ", ".join(f'"{c}"' for c in select_real)
            df = pd.read_sql(f"SELECT {cols_sql} FROM medical_data", conn)
    ...
    if "Период" in df.columns:
        ...
        df["Month_Num"] = df["dt"].dt.month.astype("int8")
        df["Quarter_Num"] = df["dt"].dt.quarter.astype("int8")
        df["Month_Name"] = df["Month_Num"].map(MONTHS_RU).astype("category")
        df["Quarter_Name"] = df["Quarter_Num"].map(QUARTERS_RU).astype("category")
        df["Month_Str"] = df["dt"].dt.strftime("%Y-%m")

    if "Сумма" in df.columns and df["Сумма"].dtype == object:
        df["Сумма"] = (
            pd.to_numeric(
                df["Сумма"].astype(str).str.replace(",", ".", regex=False).str.replace(" ", "", regex=False),
                errors="coerce",
            )
            .fillna(0.0)
            .astype("float64")
    ...
    for cat_col in ["Наименование отделения", "Наименование профиля", "Код Услуги"]:
...
@contextmanager
def duck_connection(df: pd.DataFrame) -> Iterator[duckdb.DuckDBPyConnection]:
    """Контекстный менеджер DuckDB с гарантированным закрытием.

    Yields:
        Активное in-memory соединение с зарегистрированным df.
    """
...
def apply_beautiful_layout(
    ...
    """Применяет единый адаптивный стиль к фигуре Plotly.

    Args:
        fig: Объект фигуры Plotly.
        theme: 'light' или 'dark'.
        x_range, tickvals, ticktext: опциональные настройки оси X.
    """
    palette = THEME_COLORS.get(theme, THEME_COLORS["light"])
    text_color, grid_color, card_bg = (
        palette["text"],
        palette["grid"],
        palette["card"],
    )
    ...
    if tickvals is not None and ticktext is not None:
        xaxis_config.update(tickmode="array", tickvals=tickvals, ticktext=ticktext)
...
def build_period_comparison(
    base_filters: tuple,
    compare_filters: tuple,
) -> dict:
    """Сравнивает два произвольных периода по ключевым метрикам (PoP)"""
    df = get_optimized_data()
    if df.empty:
        return {}

    def metrics_for(years, quarters, months) -> dict:
        where = build_where_clause(years, quarters, months, None, None, None)
        with duck_connection(df) as conn:
            row = conn.execute(
                f"""
                SELECT
                    COALESCE(SUM(Сумма), 0) AS total_sum,
                    COUNT(*) AS total_mes
                FROM (
                    SELECT DISTINCT "Номер ИБ", "Код Услуги", dt, Сумма
                    FROM df WHERE {where}
                )
                """
            ).fetchone()
            patients = conn.execute(
                f'SELECT COUNT(DISTINCT "ИД пациента в версии счета") '
                f"FROM df WHERE {where}"
            ).fetchone()[0] or 0
        total_sum, total_mes = row[0] or 0, row[1] or 0
        avg_check = total_sum / patients if patients else 0
        return {
            "sum": total_sum,
            "mes": total_mes,
            "patients": patients,
    ...
    base = metrics_for(*base_filters)
    comp = metrics_for(*compare_filters)

    def delta(key):
        b, c = base[key], comp[key]
        diff = b - c
        pct = (diff / c * 100) if c else 0
        return {"diff": diff, "pct": pct}

    return {
        "base": base,
        "compare": comp,
        "deltas": {k: delta(k) for k in ("sum", "mes", "patients", "avg_check")},
    }

def render_pop_result(result: dict, base_label: str, comp_label: str) -> html.Div:
    """Строит визуальный блок сравнения двух периодов."""
    if not result:
        return html.Div("Нет данных для сравнения.")

    rows = [
        ("ВЫРУЧКА", "sum", True),
        ("СРЕДНИЙ ЧЕК", "avg_check", True),
        ("ПАЦИЕНТЫ", "patients", False),
        ("УСЛУГИ (МЭС)", "mes", False),
    ]
    cards = []
    for title, key, is_money in rows:
        b = result["base"][key]
        c = result["compare"][key]
        d = result["deltas"][key]
        ...
        positive = d["diff"] >= 0
        color = "#01B574" if positive else "#E11D48"
        bg = "rgba(1,181,116,0.1)" if positive else "rgba(225,29,72,0.1)"
        icon = "fa-arrow-up" if positive else "fa-arrow-down"
                ...
                [
                    ...
                    html.Div(
                        [
                            html.Div(
                                [
                                    html.Span(comp_label, style={"fontSize": "11px", "color": "var(--text-muted)", "display": "block"}),
                                    html.B(fmt(c), style={"fontSize": "16px"}),
                                ],
                                style={"flex": 1},
                            ),
                            html.Div(
                                [
                                    html.Span(base_label, style={"fontSize": "11px", "color": "var(--text-muted)", "display": "block"}),
                                    html.B(fmt(b), style={"fontSize": "16px", "color": "var(--primary)"}),
                                ],
                                style={"flex": 1},
                            ),
                            html.Div(
                                [
                                    html.Span(
                                        [html.I(className=f"fas {icon}", style={"marginRight": "5px"}), f"{sign}{fmt(d['diff'])}"],
                                        style={"display": "block", "fontWeight": "bold", "fontSize": "13px"},
                                    ),
                                    html.Span(f"({sign}{d['pct']:.1f}%)", style={"fontSize": "11px"}),
                                ],
                                style={"flex": 1, "textAlign": "right", "color": color, "backgroundColor": bg, "padding": "8px 12px", "borderRadius": "10px"},
                            ),
                        ],
...
def build_tab_1_data(
    ...
    where_clause = build_where_clause(
        years, quarters, months, depts, profiles, mes_list, patient
    )
    fig = go.Figure()
    ...
    # --- ТВОЯ ЛОГИКА С DISTINCT ДЛЯ ТОЧНЫХ СУММ ---
        total_sum = conn.execute(f"""
            SELECT SUM(Сумма) 
            FROM (
                SELECT DISTINCT "Номер ИБ", "Код Услуги", dt, Сумма 
                FROM df 
        ...
        total_patients = (
            conn.execute(
                f'SELECT COUNT(DISTINCT "ИД пациента в версии счета") FROM df WHERE {where_clause}'
            ).fetchone()[0]
            or 0
        )
        ...
        total_mes = conn.execute(f"""
            SELECT COUNT(*) 
            FROM (
                SELECT DISTINCT "Номер ИБ", "Код Услуги", dt 
                FROM df 
                WHERE {where_clause}
            )
            """).fetchone()[0] or 0
        ...
        active_depts = (
            conn.execute(
                f'SELECT COUNT(DISTINCT "Наименование отделения") FROM df WHERE {where_clause}'
            ).fetchone()[0]
            or 0
        )

        dates_info = conn.execute(
            f"SELECT MIN(dt), MAX(dt) FROM df WHERE {where_clause}"
        ).fetchone()
        ...
        if dates_info[0] is not None:
            ...
            unique_dates_df = conn.execute(
                f"SELECT DISTINCT dt FROM df WHERE {where_clause} AND dt IS NOT NULL ORDER BY dt"
            ).df()
            ...
            if group_by_col in df.columns:
                if metric == "sum":
                    query = f'SELECT dt, "{group_by_col}", SUM(Сумма) as val FROM (SELECT DISTINCT "Номер ИБ", "Код Услуги", dt, "{group_by_col}", Сумма FROM df WHERE {where_clause}) GROUP BY dt, "{group_by_col}"'
                elif metric == "count_patients":
                    query = f'SELECT dt, "{group_by_col}", COUNT(DISTINCT "ИД пациента в версии счета") as val FROM df WHERE {where_clause} GROUP BY dt, "{group_by_col}"'
                else:
                    query = f'SELECT dt, "{group_by_col}", COUNT(*) as val FROM df WHERE {where_clause} GROUP BY dt, "{group_by_col}"'
            ...
            if "trend" in locals() and not trend.empty:
                if metric == "sum":
                    monthly = conn.execute(
                        f'SELECT Month_Str, SUM(Сумма) as val FROM (SELECT DISTINCT "Номер ИБ", "Код Услуги", Month_Str, Сумма FROM df WHERE {where_clause}) GROUP BY Month_Str ORDER BY Month_Str'
                    ).df()
                    lbl, fmt = "Общая сумма", lambda x: f"{x:,.2f} ₽".replace(",", " ")
                elif metric == "count_patients":
                    monthly = conn.execute(
                        f'SELECT Month_Str, COUNT(DISTINCT "ИД пациента в версии счета") as val FROM df WHERE {where_clause} GROUP BY Month_Str ORDER BY Month_Str'
                    ).df()
                    ...
                    monthly = conn.execute(
                        f"SELECT Month_Str, COUNT(*) as val FROM df WHERE {where_clause} GROUP BY Month_Str ORDER BY Month_Str"
                    ).df()
...
def build_tab_2_data(
    ...
    with duck_connection(df) as conn:
        ...
        has_data = (
            conn.execute(f"SELECT COUNT(*) FROM df WHERE {where_clause}").fetchone()[0] > 0
        )
        if not has_data:
            return go.Figure(), go.Figure()
        ...
        if "Month_Str" in df.columns and "Наименование отделения" in df.columns:
            if metric == "sum":
                query_hm = f'SELECT "Наименование отделения", "Month_Str", SUM(Сумма) as val FROM (SELECT DISTINCT "Номер ИБ", "Код Услуги", "Month_Str", "Наименование отделения", Сумма FROM df WHERE {where_clause}) GROUP BY "Наименование отделения", "Month_Str"'
            elif metric == "count_patients":
                query_hm = f'SELECT "Наименование отделения", "Month_Str", COUNT(DISTINCT "ИД пациента в версии счета") as val FROM df WHERE {where_clause} GROUP BY "Наименование отделения", "Month_Str"'
            else:
                query_hm = f'SELECT "Наименование отделения", "Month_Str", COUNT(*) as val FROM df WHERE {where_clause} GROUP BY "Наименование отделения", "Month_Str"'
        ...
        if set(["Наименование отделения", "Код Услуги"]).issubset(df.columns):
            ...
            for col_name in ["Наименование отделения", "Код Услуги"]:
                if str(df_sun[col_name].dtype) == 'category':
                    df_sun[col_name] = df_sun[col_name].astype(str)
            ...
            sunburst_fig.update_traces(
                maxdepth=2,
                pathbar=dict(
                    visible=True, textfont=dict(size=14, family="Inter", color="#1b2559")
                ),
                root_color="#e9edf7",
...
@cache.memoize(timeout=600)
def build_abc_data(years, quarters, months, depts, profiles, mes_list, group_by):
    df = get_optimized_data()
    if df.empty or "Сумма" not in df.columns or group_by not in df.columns:
        return pd.DataFrame()
    where_clause = build_where_clause(
        years, quarters, months, depts, profiles, mes_list
    )
    with duck_connection(df) as conn:
        has_data = (
            conn.execute(f"SELECT COUNT(*) FROM df WHERE {where_clause}").fetchone()[0] > 0
        ...
        query_abc = f"""
            SELECT "{group_by}" AS key_col, SUM(Сумма) as Сумма, COUNT(DISTINCT "Номер ИБ") as Кол_во_услуг
            FROM (
                SELECT DISTINCT "Номер ИБ", "Код Услуги", "{group_by}", Сумма
                FROM df WHERE {where_clause}
            )
            GROUP BY "{group_by}" ORDER BY Сумма DESC
        """
        return conn.execute(query_abc).df()
...
def build_where_clause(
    years, quarters, months, depts, profiles, mes_list, patient=None
):
    conditions = ["1=1"]
    if years:
        conditions.append(f'"Year" IN ({", ".join(map(str, years))})')
    if quarters:
        conditions.append(f'"Quarter_Name" IN ({clean_sqls(quarters)})')
    if months:
        conditions.append(f'"Month_Name" IN ({clean_sqls(months)})')
    if depts:
        conditions.append(f'"Наименование отделения" IN ({clean_sqls(depts)})')
    if profiles:
        conditions.append(f'"Наименование профиля" IN ({clean_sqls(profiles)})')
    if mes_list:
        conditions.append(f'"Код Услуги" IN ({clean_sqls(mes_list)})')
...
# 🚀 0. ОБНОВЛЕННЫЙ CALLBACK ДЛЯ ФИЛЬТРОВ (Чтобы не были пустые)
@app.callback(
    [
...
def update_tab_1_router(
    ...
    if trig == "btn-reset-all-filters":
        years, quarters, months, depts, profiles, mes_list, patient, yoy_toggle = (
            None,
            None,
            None,
            None,
    ...
    fig, title, kpi_sum, kpi_pat, kpi_mes, kpi_depts, insights = build_tab_1_data(
        to_tuple(years),
        to_tuple(quarters),
        to_tuple(months),
        to_tuple(depts),
    ...
    def add_chip(label, value_text):
        chips.append(
            html.Div(
                [
                    html.Span(
                        label + ":", style={"fontWeight": 600, "marginRight": "4px"}
                    ),
    ...
    if years:
        add_chip("Год", ", ".join(map(str, years)))
    if quarters:
        add_chip("Квартал", ", ".join(quarters))
    if months:
        add_chip("Месяц", ", ".join(months))
    if depts:
        add_chip("Отделения", f"{len(depts)} выбрано")
    if profiles:
        add_chip("Профили", f"{len(profiles)} выбрано")
    if mes_list:
        add_chip("Коды услуги", f"{len(mes_list)} выбрано")
    if patient:
        add_chip("ИБ", str(patient))
    if yoy_toggle and True in yoy_toggle:
        add_chip("YoY", "Включено")
    return fig, title, kpi_sum, kpi_pat, kpi_mes, kpi_depts, insights, chips


# 🚀 2. ВКЛАДКА 2 (TREEMAP И ХИТМАП С DISTINCT СУММАМИ)
@app.callback(
...
def update_table_only(
    ...
    trig = ctx.triggered[0]["prop_id"].split(".")[0] if ctx.triggered else None
    if trig == "btn-reset-all-filters":
        years, quarters, months, depts, profiles, mes_list, patient, heatmap_click = (
            None,
            None,
            None,
            None,
    ...
    with duck_connection(df) as conn:
        ...
        if "Сумма" in df.columns:
            query = f'SELECT {cols_str}, COUNT(*) as Кол_во_услуг, ROUND(SUM(Сумма), 2) as Сумма FROM (SELECT DISTINCT "Номер ИБ", "Код Услуги", "Сумма", {cols_str} FROM df WHERE {where_clause}) GROUP BY {cols_str} ORDER BY Сумма DESC LIMIT 1000'
        else:
...
# 🚀 4. ABC-АНАЛИЗ С ИНСАЙТАМИ (С DISTINCT)
@app.callback(
...
def update_abc_analysis(
    ...
    if "key_col" in df_abc.columns:
        df_abc.rename(columns={"key_col": group_by}, inplace=True)
    ...
    if total_sum > 0:
        ...
        df_abc["Доля_выручки"], df_abc["Накопленная_доля"], df_abc["Группа ABC"] = (
            "0%",
            "0%",
            "C",
        )
    ...
    group_stats = (
        df_abc.groupby("Группа ABC")
        .agg(total_sum=("Сумма", "sum"), items=("Группа ABC", "count"))
        .reset_index()
    )
    summary_items = []
    for g, label, color in [
        ("A", "A — ключевые позиции", "#01B574"),
        ("B", "B — поддерживающие", "#FF7D00"),
        ("C", "C — длинный хвост", "#E11D48"),
    ]:
        row = group_stats[group_stats["Группа ABC"] == g]
        if row.empty:
            continue
        s, n = float(row["total_sum"].iloc[0]), int(row["items"].iloc[0])
        pct = (s / total_sum * 100) if total_sum > 0 else 0
        summary_items.append(
            html.Div(
                [
                    html.Span(label, style={"fontWeight": "700", "marginRight": "6px"}),
                    html.Span(f"{n} позиций — {pct:.1f}% выручки"),
                ],
                style={
                    "padding": "6px 10px",
                    "borderRadius": "999px",
    ...
    abc_summary = html.Div(
        [
            html.Div(
                "Фокус на группе A позволит контролировать большую часть выручки.",
    ...
    return html.Div([abc_summary, ag_grid]), dash.no_update


# 🚀 5. DRILLDOWN: СТАТИСТИКА МЕДИАНЫ И ТОП-3 ПОДСВЕТКА
@app.callback(
    [
...
def drilldown_modal(
    ...
    if clickData and trigger_id == "filtered-click-data":
        ...
        mask &= df["dt"].dt.strftime("%Y-%m-%d") == clicked_date.split(" ")[0]
        mask &= df[group_by_col].astype(str).str.strip() == str(clicked_group).strip()
        df_filtered = df[mask]
        drill_col, drill_label = (
            ("Код Услуги", "МЭС")
            if group_by_col == "Наименование отделения"
            else ("Наименование отделения", "отделение")
        )

        if metric == "sum":
            df_unique = df_filtered.drop_duplicates(
                subset=["Номер ИБ", "Код Услуги", drill_col, "Сумма"]
            )
            total_val = df_unique["Сумма"].sum()
            fmt_val = f"{total_val:,.2f} ₽".replace(",", " ").replace(".", ",")
            breakdown = (
                df_unique.groupby(drill_col, observed=True)["Сумма"].sum().reset_index()
            )
        ...
        median_val = breakdown[val_col].median() if not breakdown.empty else 0
        top3_names = set(
            top_n.sort_values(val_col, ascending=False).head(3)[drill_col].tolist()
        )
        ...
        short_labels = top_n_plot[drill_col].apply(
        ...
        colors_bar = [
            "#FF7D00" if name in top3_names else "#7280FA"
            for name in top_n_plot[drill_col]
        ]
        fig_modal = go.Figure(
            go.Bar(
        ...
        if median_val > 0:
            fig_modal.add_vline(
                ...
                line_width=2,
                line_dash="dash",
                line_color="var(--grid-color)",
                annotation_text="Медиана",
                annotation_position="top right",
                annotation_font_color="var(--text-muted)",
            )
        ...
        return (
            True,
            f"Детальный анализ: {clicked_group}",
            html.Div(
                [
                    summary_text,
                    html.P(
...
# 🚀 6. МОДАЛЬНОЕ ОКНО YOY С DISTINCT
@app.callback(
    [
    ...
    if trig == "f-yoy" and yoy_val and True in yoy_val:
        ...
        if df.empty:
            return (
                ...
                html.Div("Нет данных в базе"),
                dash.no_update,
            )

        max_year_overall = int(df["Year"].max())
        target_year = max(years) if years else max_year_overall
        cy, py = target_year, target_year - 1
        where_base = build_where_clause(
            None, quarters, months, depts, profiles, mes_list, patient
        )

        with duck_connection(df) as conn:

            query_totals = f"""
                SELECT "Year", SUM(Сумма) as total_sum, COUNT(DISTINCT "Номер ИБ") as total_patients, COUNT(*) as total_mes
                FROM (SELECT DISTINCT "Номер ИБ", "Код Услуги", "Year", Сумма FROM df WHERE {where_base} AND "Year" IN ({cy}, {py}))
                GROUP BY "Year"
            """
            ...
            query_depts = f"""
                WITH dept_stats AS (
                    SELECT "Наименование отделения" as dept, SUM(CASE WHEN "Year" = {cy} THEN Сумма ELSE 0 END) as sum_cy, SUM(CASE WHEN "Year" = {py} THEN Сумма ELSE 0 END) as sum_py
                    FROM (SELECT DISTINCT "Номер ИБ", "Код Услуги", "Наименование отделения", "Year", Сумма FROM df WHERE {where_base} AND "Year" IN ({cy}, {py}))
                    GROUP BY "Наименование отделения"
                )
                SELECT dept, sum_cy, sum_py FROM dept_stats ORDER BY sum_cy DESC LIMIT 3
            """
            res_depts = conn.execute(query_depts).df()

            query_services = f"""
                WITH mes_stats AS (
                    SELECT "Код Услуги" as mes, SUM(CASE WHEN "Year" = {cy} THEN Сумма ELSE 0 END) as sum_cy, SUM(CASE WHEN "Year" = {py} THEN Сумма ELSE 0 END) as sum_py
                    FROM (SELECT DISTINCT "Номер ИБ", "Код Услуги", "Year", Сумма FROM df WHERE {where_base} AND "Year" IN ({cy}, {py}))
                    GROUP BY "Код Услуги"
                )
                SELECT mes, sum_cy, sum_py, (sum_cy - sum_py) as growth FROM mes_stats ORDER BY growth DESC LIMIT 3
            """
            ...
            for _, row in res_totals.iterrows():
                ...
                if y in stats:
                    stats[y] = {
                        "sum": row["total_sum"],
                        "pat": row["total_patients"],
                        "mes": row["total_mes"],
                    }

            avg_check_cy = (
                stats[cy]["sum"] / stats[cy]["pat"] if stats[cy]["pat"] > 0 else 0
            )
            avg_check_py = (
                stats[py]["sum"] / stats[py]["pat"] if stats[py]["pat"] > 0 else 0
            )
            ...
            def make_yoy_row(title, val_cy, val_py, is_currency=False):
                fmt = lambda x: (
                    ...
                    if is_currency
                    else f"{int(x):,.0f} шт.".replace(",", " ")
                )
                diff = val_cy - val_py
                pct = (diff / val_py * 100) if val_py > 0 else 0
                color, bg_color, icon, sign = (
                    ("#01B574", "rgba(1,181,116,0.1)", "fa-arrow-up", "+")
                    if diff >= 0
                    else ("#E11D48", "rgba(225,29,72,0.1)", "fa-arrow-down", "")
                )
            ...
            if res_depts.empty:
                ...
                for _, row in res_depts.iterrows():
                    dept_components.append(
                        make_compact_yoy_row(
                            str(row["dept"]), row["sum_cy"], row["sum_py"], is_currency=True
                        )
                    )

            service_components = [
                html.H5(
                    "📈 Топ-3 быстрорастущие услуги (по приросту выручки)",
                    style={
                        "fontWeight": "800",
                        "color": "var(--text-main)",
                        "marginTop": "25px",
            ...
            modal_body = html.Div(
                [
                    html.Div(
                        f"Анализ проводится с учетом текущих фильтров. Год для сравнения выбран автоматически.",
                        style={
                            "color": "var(--text-muted)",
                            "marginBottom": "20px",
                            "fontSize": "13px",
                    ...
                    make_yoy_row(
                        "СРЕДНИЙ ЧЕК ПАЦИЕНТА", avg_check_cy, avg_check_py, is_currency=True
                    ),
                    make_yoy_row(
                        "УНИКАЛЬНЫХ ПАЦИЕНТОВ", stats[cy]["pat"], stats[py]["pat"]
                    ),
                    make_yoy_row(
                        "ОКАЗАННЫХ УСЛУГ (МЭС)", stats[cy]["mes"], stats[py]["mes"]
                    ),
...
# 🚀 7. СБРОС ФИЛЬТРОВ И ВЫВОД ВЫДЕЛЕННЫХ ТОЧЕК
@app.callback(
    [
        Output("f-year", "value"),
...
@app.callback(
    [
        ...
        Output("main-line-chart", "selectedData", allow_duplicate=True),
    ],
    [Input("filtered-selected-data", "data"), Input("btn-close-modal", "n_clicks")],
    [State("f-group-by", "value")],
    prevent_initial_call=True,
)
def show_selected_points_data(selected_data, close_clicks, group_by_col):
    ...
    if (
        not selected_data
        or "points" not in selected_data
        or len(selected_data["points"]) == 0
    ):
    ...
    for p in selected_data["points"]:
        ...
        c_data_str = str(c_data)
        if "|" in c_data_str:
            group_val = c_data_str.split("|")[1]
        else:
            group_val = c_data_str
            
        safe_grp = str(group_val).replace("'", "''")
        conditions.append(
            f"(CAST(dt AS DATE) = '{dt_val}' AND \"{group_by_col}\" = '{safe_grp}')"
        )
    ...
    with duck_connection(df) as conn:
        query = (
            'SELECT ROUND(SUM(Сумма), 2), COUNT(*) FROM '
            '(SELECT DISTINCT "Номер ИБ", "Код Услуги", "Сумма" '
            f"FROM df WHERE {where_points})"
        )
        res = conn.execute(query).fetchone()

    total_sum, total_count = res[0] or 0, res[1] or 0
    content = html.Div(
        [
            html.Div(
                [
                    html.Span("Выделено точек: ", style={"color": "var(--text-muted)"}),
                    html.B(f"{len(selected_data['points'])}"),
                ]
            ),
            html.Div(
                [
                    html.Span("Количество услуг: ", style={"color": "var(--text-muted)"}),
                    html.B(f"{total_count:,} ед.".replace(",", " ")),
                ]
            ),
            html.Div(
                [
                    html.Span("Общая сумма: ", style={"color": "var(--text-muted)"}),
                    html.B(
                        f"{total_sum:,.2f} ₽".replace(",", " ").replace(".", ","),
                        style={"color": "#01B574", "fontSize": "16px"},
                    ),
                ]
            ),
        ]
    )
    return {"display": "block"}, content, dash.no_update
...
@app.callback(
    [
        ...
        Output("f-patient", "value", allow_duplicate=True),
    ],
    [
        Input("f-patient", "value"), 
        Input("close-patient-modal", "n_clicks"),
        Input("patient-modal", "is_open")
    ],
    prevent_initial_call=True,
)
def toggle_patient_modal(patient_id, close_clicks, is_open):
    ctx = dash.callback_context
    if not ctx.triggered:
        return dash.no_update, dash.no_update, dash.no_update, dash.no_update
        
    trigger_id = ctx.triggered[0]["prop_id"].split(".")[0]
    if trigger_id == "close-patient-modal" or (trigger_id == "patient-modal" and not is_open):
        return False, dash.no_update, dash.no_update, ""
    ...
    if trigger_id == "f-patient" and patient_id and str(patient_id).strip() != "":
        ...
        if clean_id.upper() in ["000", "BOSS", "БОСС", "42"]:
            ...
            return True, "Секретный уровень", easter_egg_msg, dash.no_update
        ...
        if df.empty or "Номер ИБ" not in df.columns:
            return (
                True,
                "Ошибка",
                html.Div("Нет данных в базе.", style={"padding": "20px"}),
                dash.no_update
            )

        mask = df["Номер ИБ"].astype(str).str.contains(clean_id, case=False, na=False)
        pat_df = df[mask].copy()

        if pat_df.empty:
            not_found_msg = html.Div(
                [
                    html.I(
                        className="fas fa-exclamation-circle",
                        style={"marginRight": "10px"},
                    ),
                    f"Пациент с ИБ «{clean_id}» не найден.",
                ],
                style={
                    "padding": "20px",
                    "color": "#E11D48",
            ...
            return True, "Результат поиска", not_found_msg, dash.no_update
        ...
        valid_dates = pat_df["dt"].dropna() if "dt" in pat_df.columns else pd.Series()
        if not valid_dates.empty:
            min_dt, max_dt = valid_dates.min(), valid_dates.max()
            period_str = f"{MONTHS_RU.get(min_dt.month, '')} {min_dt.year} - {MONTHS_RU.get(max_dt.month, '')} {max_dt.year}"
        else:
            period_str = "Н/Д"
        ...
        if "dt" in pat_df.columns:
            ...
            pat_df["period_label"] = pat_df["dt"].dt.strftime("%Y-%m")
        else:
            pat_df["period_label"] = "Период не указан"

        for period, grp in pat_df.groupby("period_label", sort=False):
            period_total = grp["Сумма"].sum() if "Сумма" in grp.columns else 0
            period_label_readable = (
                f"{MONTHS_RU.get(grp['dt'].iloc[0].month, '')} {grp['dt'].iloc[0].year}"
                if "dt" in grp.columns and pd.notnull(grp["dt"].iloc[0])
                else period
            )
            header = html.Div(
                [
                    ...
                    html.Span(
                        f"{len(grp)} услуг, {period_total:,.2f} ₽".replace(
                            ",", " "
                        ).replace(".", ","),
                        style={
                            "marginLeft": "auto",
            ...
            history_items.append(header)

            for _, row in grp.iterrows():
                mes = row.get("Код Услуги", "Н/Д")
                formatted_summ = f"{row.get('Сумма', 0):,.2f} ₽".replace(
                    ",", " "
                ).replace(".", ",")
                item = html.Div(
                    [
                ...
                dbc.Row(
                    [
                        dbc.Col(
                            create_kpi_card(
                                "ОБЩИЙ ЧЕК",
                                f"{total_spent:,.2f} ₽".replace(",", " ").replace(
                                    ".", ","
                                ),
                                "fas fa-money-bill",
                                "#01B574",
        ...
        return True, f"Карточка истории болезни: {clean_id}", modal_body, dash.no_update
        
    return dash.no_update, dash.no_update, dash.no_update, dash.no_update


@app.callback(
    Output("auto-insights-container", "children"),
    [
        Input("btn-apply-filters", "n_clicks"),
        Input("btn-reset-all-filters", "n_clicks"),
...
def build_auto_insights(
    ...
    with duck_connection(df) as conn:
        ...
        top_month = conn.execute(
            f'SELECT Month_Str, SUM(Сумма) AS s FROM (SELECT DISTINCT "Номер ИБ", "Код Услуги", Month_Str, Сумма FROM df WHERE {where_base}) GROUP BY Month_Str ORDER BY s DESC LIMIT 1'
        ).fetchone()
        top_dept = conn.execute(
            f'SELECT "Наименование отделения", SUM(Сумма) AS s FROM (SELECT DISTINCT "Номер ИБ", "Код Услуги", "Наименование отделения", Сумма FROM df WHERE {where_base}) GROUP BY "Наименование отделения" ORDER BY s DESC LIMIT 1'
        ).fetchone()

        active_depts = (
            conn.execute(
                f'SELECT COUNT(DISTINCT "Наименование отделения") FROM df WHERE {where_base}'
            ).fetchone()[0]
            or 0
        )
        ...
        def insight_card(title, desc, icon, color):
            return html.Div(
                [
                    html.Div(
                    ...
                    html.Div(
                        [
                            html.Div(
                                title, style={"fontWeight": "700", "fontSize": "13px"}
                            ),
                            html.Div(
                                desc,
        ...
        if top_month:
            cards.append(
                insight_card(
                    ...
                    f"{top_month[0]}: {top_month[1]:,.2f} ₽".replace(",", " ").replace(
                        ".", ","
                    ),
                    "fas fa-calendar-alt",
                    "#4318FF",
        ...
        if top_dept:
            cards.append(
                insight_card(
                    "Топ-отделение по выручке",
                    f"{str(top_dept[0])[:25]}: {top_dept[1]:,.2f} ₽".replace(
                        ",", " "
                    ).replace(".", ","),
                    "fas fa-hospital",
                    "#01B574",
        ...
        cards.append(
            insight_card(
                "Активных отделений",
                f"{active_depts} за выбранный период",
...
def execute_custom_sql(n_clicks, query, theme):
    ...
    try:
        ...
        conn = duckdb.connect()
        conn.register("medical_data", df)
        df_sql = conn.execute(safe_query).df()
        ...
        for col in df_sql.columns[1:]:
            if pd.api.types.is_numeric_dtype(df_sql[col]):
                has_numeric = True
                if df_sql[col].max() > max_y_value:
                    max_y_value = df_sql[col].max()
                c = colors[color_idx % len(colors)]
                y_format = (
                    "%{y:,.2f} ₽"
                    if "сумм" in str(col).lower() or "sum" in str(col).lower()
                    else "%{y:,.0f}"
                )
                fig.add_trace(
                    go.Scatter(
                        ...
                        customdata=["SQL|None"] * len(df_sql),
                    )
                )
                color_idx += 1
        ...
        ticktext = [
            str(v) if len(str(v)) <= 35 else str(v)[:32] + "..." for v in df_sql[x_col]
        ]
        ...
        return fig, "Аналитика", "", sql_insight
...
app.clientside_callback(
    """
    function(y, q, m, d, p, mes, pat, metric, grp, applyClicks) {
        const trig = dash_clientside.callback_context.triggered.map(t => t.prop_id);
        const base = {
            "width": "100%", "border": "none", "borderRadius": "16px",
            "padding": "14px", "fontWeight": "600", "marginTop": "5px",
            "boxShadow": "0px 10px 20px rgba(1, 181, 116, 0.2)",
            "backgroundColor": "#01B574"
        };

        if (trig.includes('btn-apply-filters.n_clicks')) {
            return [base, ""];
        }
        return [{...base, "backgroundColor": "#F59E0B"}, "btn-apply-dirty"];
    }
    """,
    [
        Output("btn-apply-filters", "style"),
        Output("btn-apply-filters", "className"),
    ],
    [
        Input("f-year", "value"),
        Input("f-quarter", "value"),
...
app.clientside_callback(
    ...
    function(n1, n2, n3) {
        ...
        if (trig.length > 0) {
            ...
            if (graphDiv && window.Plotly) {
                ...
                // Принудительно возвращаем цвета всем линиям
                window.Plotly.restyle(graphDiv, {'selectedpoints': [null]}, idx);
            }
            return null; // Очищаем данные о выделении в Dash
        }
        return window.dash_clientside.no_update;
    }
    """,
    ...
    [
        ...
        Input("btn-reset-selection", "n_clicks"),
    ],
    prevent_initial_call=True,
)

app.clientside_callback(
    "function(n_clicks, current_style) { if (!n_clicks) return window.dash_clientside.no_update; let new_style = {...current_style}; if (new_style.height === '500px') { new_style.height = '1300px'; } else { new_style.height = '500px'; } return new_style; }",
...
app.clientside_callback(
    "function(selectedData) { if (!selectedData || !selectedData.points || selectedData.points.length === 0) { return null; } if (window.isShiftPressed || selectedData.points.length > 1) { return selectedData; } return window.dash_clientside.no_update; }",
    ...
    function(n_clicks) { 
        if (n_clicks > 0) { 
            ...
            if (element) {
                var opt = {
                    margin:       10,
                    filename:     'Clinical_Dashboard_Report.pdf',
...
app.clientside_callback(
    ...
    function(n_clicks, current_theme) {
        ...
        const new_icon = new_theme === 'dark' ? 'fas fa-sun' : 'fas fa-moon';
        ...
        if os.path.exists(update_dir) and os.path.isdir(update_dir):
            ...
            if any(f.lower().endswith('.exe') for f in files):
                btn_style = {
                    ...
                    "backgroundColor": "#01B574", "color": "white",
                    "border": "none", "borderRadius": "12px",
                    "padding": "8px 16px", "fontWeight": "800",
                    "marginRight": "20px", "cursor": "pointer",
                    "animation": "pulse-green 2s infinite",
                    "boxShadow": "0 10px 20px rgba(1, 181, 116, 0.3)"
                }
                
                if is_currently_open:
                    return dash.no_update, btn_style
                return True, btn_style
    except Exception:
        pass
...
@app.callback(
    Output("f-patient", "value", allow_duplicate=True),
    Input("interactive-grid", "cellClicked"),
    prevent_initial_call=True,
)
def grid_patient_drilldown(cell):
    """Открывает карточку пациента при клике на ячейку 'Номер ИБ' в таблице"""
    if not cell:
        raise dash.exceptions.PreventUpdate
    if cell.get("colId") != "Номер ИБ":
        raise dash.exceptions.PreventUpdate
...
@app.callback(
    [Output("pop-a-year", "options"), Output("pop-b-year", "options"),
     Output("pop-a-month", "options"), Output("pop-b-month", "options")],
    Input("main-tabs", "active_tab"),
)
def fill_pop_dropdowns(_):
    df = get_optimized_data()
    if df.empty:
        return [], [], [], []
    years = sorted(df["Year"].unique()) if "Year" in df.columns else []
    opts_y = [{"label": str(y), "value": int(y)} for y in years]
    opts_m = [{"label": MONTHS_RU[m], "value": MONTHS_RU[m]} for m in sorted(df["Month_Num"].unique())] if "Month_Num" in df.columns else []
...
def run_pop(n_run, n_close, is_open, y_a, m_a, y_b, m_b):
    ...
    result = build_period_comparison(base_filters, comp_filters)
    
    label_a = f"{m_a or 'Весь год'} {y_a}"
...
app.clientside_callback(
    ...
    function(n_clicks) {
        if (n_clicks > 0) {
            window.close();
            alert("Пожалуйста, закройте эту вкладку вручную (на крестик), чтобы программа обновилась.");
        }
        return window.dash_clientside.no_update;
    }
    """,
